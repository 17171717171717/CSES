...
int solve(int n, vi dp){
    ...
    for (int i = 0; i < nums.size(); i++) {
        int cur_digit = nums[i] - '0';
        int k_th_bit = nums.size() - i - 1;
        int below = prev_digit_pos < cur_digit ? cur_digit - 1 : cur_digit;
        ans += (cur_digit == 0) ? 0 : below * dp[k_th_bit];
        if (cur_digit == prev_digit_pos)
            return ans;
        prev_digit_pos = cur_digit;
    }
    return ans + 1; 
}
...
int memorize_dp(str &s, int cur_digit, int pre_digit, bool limit, bool leading_zero){

    // this space is done => it's like recursion, so we backtrack and memorize the dp from end to fron
    if(dp[cur_digit][pre_digit][limit][leading_zero]!=-1)
        return dp[cur_digit][pre_digit][limit][leading_zero];

    // initial cond.
    if (cur_digit==(int)s.size()){
        return dp[cur_digit][pre_digit][limit][leading_zero] = 1;
    }
    ...
    for(int iterate_digit = 0; iterate_digit <= max_iteration ; iterate_digit++){
        if(iterate_digit == pre_digit){
            ...
            if(!leading_zero) continue; // if it isn't 000xxx
            //else: 000XXX => OK
        }
        // 56789 => 789 is limited by 56 => 6 is limited by 5  ==> 56 turn go to 789 =>when iterate 7 => 8 is limit
        bool next_digit_limit = limit & (iterate_digit == (s[cur_digit]-'0'));
        bool still_leading_zero = leading_zero & (cur_digit=='0');
        ans+= memorize_dp(s, cur_digit+1, iterate_digit, next_digit_limit, still_leading_zero);
    }
    ans = dp[cur_digit][pre_digit][limit][leading_zero] ;
    return ans;
}
...
int solve2(int n){
    ...
    return memorize_dp(to_string(n), 0, 0, true, true);
}

...
signed main()
{
    ...
    while(kase--){
        ...
        for (int i = 0; i < n; i++){
            ...
            cin >> q;
            q--;
            // This process will end when we place a number in the position of an already missing number.
            // p=[4,5,3,0,0], it can be transformed into [1,2,3,4,5]
            // p=[0,5,3,0,0], it can be transformed into [1,2,3,4,5]
            // => p[p[2]] is already need to change, so we dont need to set p[5] again
            while(cur_changed.find(q) == cur_changed.end()){
                cur_changed.insert(q);
                q = v[q];
            }
            cout << cur_changed.size() << ' ';
        }
        cout << endl;
    }
    return 0;
